...
public class BigRoundTree {
    ...
    private static final float BRANCH_TIP_EXTENSION_FACTOR = 0.2f;
    ...
    private static final int BRANCH_HEIGHT_RANDOMNESS = 15;
    public static final int SUB_BRANCH_COUNT_LESS_THAN_5 = 8;
    public static final int SUB_BRANCH_COUNT_LESS_THAN_3 = 0;
    public static final int TRUNK_HEIGHT_EXT = 17;

    private static final int MAX_MASTER_BRANCHES = 16;
    private static final int BRANCH_SPACING = (TRUNK_HEIGHT - MIN_BRANCH_HEIGHT) / MAX_MASTER_BRANCHES;
    ...
    public static void generate(Level level, BlockPos origin, RandomSource random) {
        ...
        System.out.println("[BigRoundTree] Starting tree generation at: " + origin);
        ...
        System.out.println("[BigRoundTree] Tree generation complete.");
    }
...
// Precalculate evenly spaced master branch Y positions
        int[] masterBranchHeights = new int[MAX_MASTER_BRANCHES];
        for (int i = 0; i < MAX_MASTER_BRANCHES; i++) {
            masterBranchHeights[i] = MIN_BRANCH_HEIGHT + i * (TRUNK_HEIGHT - MIN_BRANCH_HEIGHT) / MAX_MASTER_BRANCHES;
        }
        ...
        for (int y = 0; y < TRUNK_HEIGHT + BRANCH_EXTRA_CAP; y++) {
            ...
            // Use precomputed master branch heights
            if (branchIndex < MAX_MASTER_BRANCHES && y == masterBranchHeights[branchIndex]) {
                ...
                System.out.println("[BigRoundTree] Generating 4 branches at Y=" + actualY);
                BlockPos branchStart = origin.offset(0, actualY, 0);
                generateBranch(level, branchStart.above(random.nextInt(BRANCH_HEIGHT_RANDOMNESS) - 1),  1,  0, baseBranchWidth, baseBranchLength, 0, random);
                generateBranch(level, branchStart.above(random.nextInt(BRANCH_HEIGHT_RANDOMNESS) - 1), -1,  0, baseBranchWidth, baseBranchLength, 0, random);
                generateBranch(level, branchStart.above(random.nextInt(BRANCH_HEIGHT_RANDOMNESS) - 1),  0,  1, baseBranchWidth, baseBranchLength, 0, random);
                generateBranch(level, branchStart.above(random.nextInt(BRANCH_HEIGHT_RANDOMNESS) - 1),  0, -1, baseBranchWidth, baseBranchLength, 0, random);
    ...
    private static void generateBranch(Level level, BlockPos start, int dirX, int dirZ, int width, int length, int depth, RandomSource random) {
        if (depth > BRANCH_DEPTH_LIMIT || branchCounter++ > MAX_TOTAL_BRANCHES) {
            System.out.println("[BigRoundTree] Branch limit hit or depth exceeded. Depth: " + depth + ", Counter: " + branchCounter);
            return;
        }
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (!subBranchCreated && i >= triggerSubBranchAt) {
                ...
                if (taperWidth < 5) subBranchCount = SUB_BRANCH_COUNT_LESS_THAN_5;
                if (taperWidth < 3) subBranchCount = SUB_BRANCH_COUNT_LESS_THAN_3;
    ...
    private static void placeCap(Level level, BlockPos pos, RandomSource random) {
        // Add 15 vertical trunk blocks first, all exempt from leaves

        for (int i = -5; i < TRUNK_HEIGHT_EXT; i++) {
            BlockPos trunkPos = pos.above(i);
            setBlock(level, trunkPos, TRUNK_BLOCK);
            branchPositions.add(trunkPos);
            leafExemptPositions.add(trunkPos);
        }

        // Then add 2 cap blocks on top of that, also exempt from leaves
        for (int i = TRUNK_HEIGHT_EXT; i < TRUNK_HEIGHT_EXT+2; i++) {
            BlockPos capPos = pos.above(i);
            BlockState cap = CAP_BLOCKS[random.nextInt(CAP_BLOCKS.length)];
            setBlock(level, capPos, cap);
            branchPositions.add(capPos);
            leafExemptPositions.add(capPos);
        }
    }
